...
class Blog extends Page {
    ...
    public articles: ReadonlyArray<WPSearchPost> = [];
    @property({type: Array, reflect: false})
    private ghost: ReadonlyArray<WPSearchPost> = [];
    ...
    public search(value: string){
        this.articles = this.ghost.filter(item => item.title.rendered.toLowerCase().indexOf(value.toLowerCase()) !== -1);
    }
